...
def extract_from_target_css(keys: list[str], target_css: str, overwrite=False):
    d = {}
    
    lines = [i.strip() for i in target_css.split("\n") if i != ""]
    for i in lines:
        parts = i.split(":")
...
basic_keys = extract_keys(template)
# print(basic_keys)

d = extract_from_target_css(basic_keys, target)
print(d)

print(len(basic_keys))
print(len(d))
